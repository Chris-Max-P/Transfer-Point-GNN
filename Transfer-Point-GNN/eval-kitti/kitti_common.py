...
def get_velodyne_path(idx, prefix, training=True, relative_path=True):  # solved: not used
...
def get_label_anno(label_path):
    ...
    annotations['name'] = np.array([x[0] for x in content if x != ['']])
    annotations['truncated'] = np.array([float(x[1]) for x in content if x != ['']])
    annotations['occluded'] = np.array([int(x[2]) for x in content if x != ['']])
    annotations['alpha'] = np.array([float(x[3]) for x in content if x != ['']])
    annotations['bbox'] = np.array(
        [[float(info) for info in x[4:8]] for x in content if x != ['']]).reshape(-1, 4)
    # dimensions will convert hwl format to standard lhw(camera) format.
    annotations['dimensions'] = np.array(
        [[float(info) for info in x[8:11]] for x in content if x != ['']]).reshape(
            -1, 3)[:, [2, 0, 1]]
    annotations['location'] = np.array(
        [[float(info) for info in x[11:14]] for x in content if x != ['']]).reshape(-1, 3)
    annotations['rotation_y'] = np.array(
        [float(x[14]) for x in content if x != ['']]).reshape(-1)
    if len(content) != 0 and len(content[0]) == 16:  # have score
        annotations['score'] = np.array([float(x[15]) for x in content if x != ['']])
    else:
...
def get_label_annos_MUT(label_folder):
    annos = []
    for label_file in os.listdir(label_folder):
        annos.append(get_label_anno(os.path.join(label_folder, label_file)))
    return annos
