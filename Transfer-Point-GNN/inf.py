...
from source.dataset_classes.dataset import Dataset, Points, get_input_features, sel_xyz_in_box3d
from source.dataset_classes.point_cloud_processing import downsample_by_random_voxel
from source.dataset_classes.data_specifics import label_map_to_double_list, get_label_map
...
from util_point_gnn.config_util import load_config, load_train_config

parser = argparse.ArgumentParser(description='Point-GNN inference')
parser.add_argument('dataset', type=str, help='Dataset to infer on')
...
parser.add_argument('--jobname', type=str, default='inf_tf')
...
meta_config = load_train_config(os.path.join(args.checkpoint_path, 'meta_config'))

DATASET = args.dataset  # changed: added args
JOBNAME = args.jobname
print("running job " + JOBNAME + ' on ' + DATASET)

if args.dataset_split_file == '':
    DATASET_SPLIT_FILE = os.path.join(DATASET_DIR, '3DOP_splits', meta_config['test_dataset'])
else:
    DATASET_SPLIT_FILE = args.dataset_split_file
if args.output_dir == '':
    OUTPUT_DIR = os.path.join(args.checkpoint_path, 'eval_mut/')
else:
    OUTPUT_DIR = args.output_dir
CHECKPOINT_PATH = args.checkpoint_path
CONFIG_PATH = os.path.join(CHECKPOINT_PATH, 'config')
assert os.path.isfile(CONFIG_PATH), 'No config file found in ' + CONFIG_PATH
...
if DATASET == 'KITTI':
    image_path = f'image/{mode}/image_2'
    point_cloud_path = f'velodyne/{mode}/velodyne/'
    calib_path = f'calib/{mode}/calib/'
    label_path = 'labels/training/label_2'  # changed: not needed in test mode => not variable
    if IS_TEST:
        label_path = ''
...
dataset = Dataset(
    ...
    os.path.join(DATASET_DIR, point_cloud_path),
...
def t_initial_vertex_features(config_input_features):
...
t_initial_vertex_features = t_initial_vertex_features(config['input_features'])
...
with tf.compat.v1.Session(graph=graph,
    ...
    for frame_idx in tqdm(range(0, NUM_TEST_SAMPLE)):
        ...
        # provide input ======================================================
        if DATASET == 'KITTI':  # changed: necessary for applying inf on MUT
            ...
            image = dataset.get_image(frame_idx)  # only used for visualization
        elif DATASET == 'MUT':
        ...
        if DATASET == 'MUT':
            downsample_size = config['downsample_by_voxel_size'] + 0.005
            critical_point = edges_list[0].shape[0]
            while critical_point > 900000:
                downsample_size = downsample_size + 0.005
                cam_rgb_points = dataset.get_velo_points(frame_idx)
                if config['downsample_by_voxel_size'] is not None:
                    cam_rgb_points = downsample_by_random_voxel(cam_rgb_points, downsample_size)
                (vertex_coord_list, keypoint_indices_list, edges_list) = \
                    graph_generate_fn(
                        cam_rgb_points.xyz, **config['runtime_graph_gen_kwargs'])
                critical_point = edges_list[0].shape[0]
        ...
        label_map = get_label_map(config['label_method'])  # changed
        ...
        print(f"Running session with edges list of {np.asarray(edges_list[0]).shape}")
        results = sess.run(fetches, feed_dict=feed_dict)
        # fetches > optimizer steps
        # feed-dict passes data to placeholders
        gnn_time = time.time()
        ...
        if box_indices.size != 0:
            ...
            for i in range(len(detection_boxes_3d_corners)):
                all_class_name = label_map_to_double_list(label_map)
                # changed
                ...
                if DATASET == 'KITTI':
                    ...
                    clip_xmax = min(xmax, 1242.0)  # image width
                    clip_ymax = min(ymax, 375.0)  # image height
                    ...
                    if truncation_rate > 0.4:
                        continue
                elif DATASET == 'MUT':  # changed: no images available in MUT
                    clip_xmin = -1
                    clip_ymin = -1
                    clip_xmax = -1
                    clip_ymax = -1
                ...
                if USE_BOX_SCORE:
                    ...
                    # Rescore or not ===========================================
                    inside_mask = sel_xyz_in_box3d(tmp_label, last_layer_points_xyz[box_indices])
            ...
            # output ===========================================================
            filename = OUTPUT_DIR + '/' + JOBNAME + '/' + dataset.get_filename(
                frame_idx) + '.txt'
            translation = dataset.get_translation(frame_idx)
            os.makedirs(os.path.dirname(filename), exist_ok=True)
            with open(filename, "w") as f:
                for pred_label in pred_labels:
                    for index, field in enumerate(pred_label):
                        if index == 11:
                            f.write(str(field + translation[1]) + ' ')
                        elif index == 12:
                            f.write(str(field + translation[2]) + ' ')
                        elif index == 13:
                            f.write(str(field + translation[0]) + ' ')
                        else:
                            f.write(str(field) + ' ')
                    f.write('\n')
                f.write('\n')
