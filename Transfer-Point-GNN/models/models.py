...
class MultiLayerFastLocalGraphModelV2(object):
    ...
    def predict(self,
        ...
        with slim.arg_scope([slim.batch_norm], is_training=is_training), \
                ...
                for idx in range(len(self._layer_configs)-1):
                    ...
                    trainable = layer_config['trainable'] if 'trainable' in layer_config else True
                ...
                predictor_trainable = predictor_config['trainable'] if 'trainable' in predictor_config else True
    ...
    def loss(self, logits, labels, pred_box, gt_box, valid_box,
        ...
        if loc_loss_type == 'huber_loss':
            ...
            loc_loss = tf.compat.v1.div_no_nan(tf.reduce_sum(mean_loc_loss),
                                               num_valid_endpoint)
            classwise_loc_loss = []
            for class_idx in range(self.num_classes):
                ...
                is_nan_mask = tf.compat.v1.is_nan(l)
        ...
        with tf.control_dependencies([tf.assert_equal(tf.compat.v1.is_nan(loc_loss),
                                                      False)]):
